...
# Note: 'BeforeAfterAvecFeaturesNormal.mat' was too large to be added to GitHub, so please talk to Reza or Christina to obtain the file!
# Accuracy and precision/recall scores for gemaps is much, much higher regardless
...
features = loadmat('BeforeAfterAvecFeaturesNormal.mat')
labels = loadmat('BeforeAfterAvecLabels.mat')


# In[3]:


main = pd.DataFrame(np.array(features['features']))
encoded = pd.DataFrame(np.array(labels['labels']))
encoded = encoded.rename(columns={ encoded.columns[0]: "label" })
...
encoded_labels = encoder.transform(encoded)
...
# Remove solvers (don't accept l1 lasso penalty), using lasso to reduce features
grid = {'C': [0.001, 0.01, 0.1, 1, 10, 100], 'tol': [1e-4, 1e-3, 1e-2]}
# Use l1 penalty to dampen effect of some features (else may not converge within max_iter)
lr = GridSearchCV(LogisticRegression(penalty='l1', max_iter=5000), param_grid=grid)
...
ann = Sequential()
ann.add(Dense(4, activation='relu', kernel_initializer='random_normal', input_dim=2267)) # First hidden layer
