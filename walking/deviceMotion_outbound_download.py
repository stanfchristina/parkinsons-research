...
queried_walking_table = syn.tableQuery(("SELECT 'recordId', 'healthCode', 'deviceMotion_walking_outbound.json.items', 'medTimepoint' FROM syn5511449 WHERE medTimepoint IN ('Immediately before Parkinson medication', 'Just after Parkinson medication (at your best)')").format("syn5511449"))
...
json_files = syn.downloadTableColumns(queried_walking_table, "deviceMotion_walking_outbound.json.items")
...
walk_json_files_temp = pd.DataFrame({"deviceMotion_walking_outbound.json.items": [i[0] + '.0' for i in items], "outbound_walk_json_file": [i[1] for i in items]})
walk_json_files_temp
...
reduced_data_set.to_csv('deviceMotion_outbound_reduced.csv')
