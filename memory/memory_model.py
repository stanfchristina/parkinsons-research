...
table = pd.read_csv('memory_activity.csv')

filtered_table = table[(table['medTimepoint'] == 'Just after Parkinson medication (at your best)') | (table['medTimepoint'] == 'Immediately before Parkinson medication')]
filtered_table = filtered_table.drop(['ROW_ID', 'ROW_VERSION', 'createdOn', 'appVersion', 'phoneInfo', 'MemoryGameResults.json.startDate', 'MemoryGameResults.json.endDate', 'MemoryGameResults.json.MemoryGameGameRecords'], axis=1)
filtered_table
...
ann = Sequential()
ann.add(Dense(4, activation='relu', kernel_initializer='random_normal', input_dim=3)) # First hidden layer
